...
| 2026-08-08 | grok | Filters page M3: dirty tracking + sticky save bar (Save/Discard/Reset all), PUT with per-field 400 mapping, 404 → «API ещё нет», snackbar «применится со следующего цикла охоты». Specs: badge appear/disappear, reset removes draft key, extend_only chip immovable, indeterminate groups. |
