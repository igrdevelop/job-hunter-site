...
| `/profile` | `/api/profile` | — | Career-profile editor (F1 read-only skeleton + F2 editable skills table — identity/questionnaire/roles/leftovers still read-only); mock-first behind `PROFILE_MOCK_FALLBACK_ENABLED`, see `docs/RESUME_PROFILE_STORE.md` |
