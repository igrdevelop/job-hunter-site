...
| `/filters` | `/api/filters` | bot `filters.yaml` / code FILTER | Job Filters page (M1 skeleton: read-only sections from GET; mock fallback while API 404); plan in `docs/FILTERS_PAGE_PLAN.md` |
...
| 2026-08-08 | grok | Filters page M1 (`docs/FILTERS_PAGE_PLAN.md`): `FiltersApi` GET/PUT + models (`FiltersPayload`/`FilterMeta`/`FiltersErrors`) + plan-shaped mock fixture (404 fallback while live API missing); lazy `/filters` route + "Job Filters" nav; read-only skeleton rendering 7 mockup sections (no §8 preview — v2) from GET effective/overrides/meta. Specs for api + component. |
